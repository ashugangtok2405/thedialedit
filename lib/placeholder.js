...
export function sortedMedia(product) {
  return [...(product.product_media || [])].sort((a, b) => a.sort_order - b.sort_order);
}

export function productImage(product) {
  const firstImage = sortedMedia(product).find((m) => m.type === "image");
  return firstImage?.url || product.image_url || placeholderImage(product);
}
