...
// Prefer a branded /api/img/<id> link (shows your own domain in the WhatsApp
// preview) over the raw Supabase storage URL, which only exists for legacy
// products saved before multi-media support.
function photoLink(product) {
  const image = sortedMedia(product).find((m) => m.type === "image");
  if (image) {
    const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || "https://thedialedit.netlify.app";
    return `${siteUrl}/api/img/${image.id}`;
  }
...
export function buildWhatsAppCheckoutUrl(lineItems, total) {
  ...
  lineItems.forEach((li, idx) => {
    ...
    const photo = photoLink(li.product);
